...
session_file = 'flash_session.session'  # حتما این فایل قبلا ساخته شده باشه و روی سرور باشه
...
async def main_loop():
    await client.start()  # فقط از فایل سشن استفاده می‌کنه، کد تایید نمیخواد
